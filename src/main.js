...
function scheduleConfigSave() {
  if (!configControlsAreValid()) return;
  clearTimeout(configSaveTimer);
  configSaveTimer = setTimeout(() => { configSaveQueue = configSaveQueue.then(() => saveConfig({ quiet: true })); }, 300);
}

function flushConfigSave() {
  clearTimeout(configSaveTimer);
  if (!configControlsAreValid()) return Promise.resolve(null);
  configSaveQueue = configSaveQueue.then(() => saveConfig({ quiet: true }));
